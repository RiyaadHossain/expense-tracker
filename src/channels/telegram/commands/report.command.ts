  ...
  bot.command("report", async (ctx) => {
    try {
        ...
        report.recentTransactions.length === 0
          ...
          : report.recentTransactions
              .map((tx, index) => {
                ...
                return (
                  `${index + 1}. *${label}* - 💵 *${formatAmount(Number(tx.amount))} ${currency}*\n` +
                  `📝 Note: ${tx.note || "No note"}\n` +
                  `🕒 Time: ${formatTime(tx.transactionAt)}`
                );
              })
              .join("\n\n");
      ...
      const message =
        ...
        `- ⚖️ Balance: *${formatAmount(report.todaySummary.balance)} ${currency}*\n` +
        ...
        `- ⚖️ Balance: *${formatAmount(report.monthSummary.balance)} ${currency}*\n` +
